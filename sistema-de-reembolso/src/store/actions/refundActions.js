...
export const getRefund = async (dispatch, statusRefund, page, quantityPerPage) => {
    try {
        const {data} = await apiRefund.get(`/reembolso/logged/list/status?statusReembolso=${statusRefund}&pagina=${page}&quantidadeDeRegistros=${quantityPerPage}`)
...
export const handleDeleteRefund = async (dispatch, idRefund) => {
    Swal.fire({
        title: 'Tem certeza que deseja excluir?',
        text: 'Você não vai poder modificar depois!',
...
export const handleUpdateRefund = async (dispatch, values, idRefund, navigate) => {
    try {
